...
export function liftRow(row: Record<string, unknown>): Record<string, unknown> {
    ...
    if (key.length > 11 && key.startsWith("identifiers") && /[A-Z]/.test(key[11])) { identifiers[key[11].toLowerCase() + key.slice(12)] = val; }
    else if (key.length > 12 && key.startsWith("purchaseUrls") && /[A-Z]/.test(key[12])) { purchaseUrls[key[12].toLowerCase() + key.slice(13)] = val; }
    else { out[key] = val; }	
  }
